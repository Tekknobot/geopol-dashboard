...
export default function DeskPage({config}:{config:DeskConfig}){
  ...
  return <main className="desk-page" data-desk={config.desk}>
    ...
    <div className="desk-ticker" aria-label="Latest headline ticker"><strong>NOW</strong><div>{stories.slice(0,8).map((story)=><a key={story.id} href={story.articleUrl} target="_blank" rel="noreferrer"><span>{story.category}</span><span data-atlas-content-translate>{story.title}</span></a>)}</div></div>
    ...
    <section className="desk-lead-grid">
      {hero?<section className="desk-hero" aria-label="Top story carousel" onTouchStart={(event)=>{heroTouchX.current=event.touches[0]?.clientX??null;}} onTouchEnd={(event)=>finishHeroSwipe(event.changedTouches[0]?.clientX??0)}>
        ...
        <div className="desk-hero-copy"><p><span>TOP STORY</span>{hero.category} · {hero.region}</p><h2 data-atlas-content-translate>{hero.title}</h2><div className="desk-hero-meta"><strong>{hero.source}</strong><time dateTime={hero.publishedAt} title={exactTime(hero.publishedAt)}>{relativeTime(hero.publishedAt,clock)}</time><span>{hero.read} read</span></div><p className="desk-hero-summary" data-atlas-content-translate>{hero.summary}</p><div className="desk-hero-actions"><a href={hero.articleUrl} target="_blank" rel="noreferrer">Read original story ↗</a><button type="button" className={savedIds.includes(hero.id)?"saved":""} onClick={()=>toggleSaved(hero)}>{savedIds.includes(hero.id)?"◆ Saved":"◇ Save"}</button></div></div>
    ...
    <section className="desk-layout">
      <div className="desk-main-column">
        <section className="desk-featured"><div className="desk-section-heading"><div><p>VISUAL EDIT</p><h2>Picture-led stories</h2></div><span>Publisher imagery · original links</span></div><div className="desk-card-rail">{stories.slice(1,7).map((story)=><article key={story.id}><a href={story.articleUrl} target="_blank" rel="noreferrer"><StoryImage story={story}/><div><p>{story.category} · {story.region}</p><h3 data-atlas-content-translate>{story.title}</h3><span>{story.source} · <time dateTime={story.publishedAt}>{relativeTime(story.publishedAt,clock)}</time></span></div></a></article>)}</div></section>
        ...
        <section className="desk-wire" id="desk-wire"><div className="desk-section-heading wire-heading"><div><p>LIVE HEADLINE WIRE</p><h2>{topic==="All"?`Latest ${config.desk}`:topic}</h2><span>{filtered.length} verified headline{filtered.length===1?"":"s"}</span></div><select value={region} onChange={(event)=>setRegion(event.target.value)} aria-label="Filter headlines by region">{regions.map((item)=><option key={item}>{item}</option>)}</select></div>{status==="partial"&&<p className="desk-feed-note">Some publishers are temporarily unavailable. Available stories retain their original links and publication times.</p>}<div className="desk-wire-list">{filtered.map((story)=><article key={story.id}><a href={story.articleUrl} target="_blank" rel="noreferrer"><span className={`status-dot ${story.level}`}/><div><p>{story.category} · {story.region}</p><h3 data-atlas-content-translate>{story.title}</h3><small>{story.source} · <time dateTime={story.publishedAt} title={exactTime(story.publishedAt)}>{relativeTime(story.publishedAt,clock)}</time> · {story.read}</small></div><b>↗</b></a><button type="button" className={`desk-wire-save ${savedIds.includes(story.id)?"saved":""}`} onClick={()=>toggleSaved(story)} aria-label={`${savedIds.includes(story.id)?"Remove":"Save"} ${story.title}`}>{savedIds.includes(story.id)?"◆":"◇"}</button></article>)}</div>{!filtered.length&&status!=="loading"&&<div className="desk-no-results"><strong>No matching headlines</strong><p>Try another topic, region or search phrase.</p><button onClick={reset}>Reset filters</button></div>}</section>
      </div>
