...
type Story = { id:number; category:string; region:string; time:string; level:"critical"|"elevated"|"watch"|"stable"; title:string; summary:string; source:string; read:string; tags:string[]; articleUrl?:string };
...
const sourceUrlForStory = (story:Story) => story.articleUrl ?? `https://www.google.com/search?q=${encodeURIComponent(`site:${sourceDomains[story.source] ?? "news.google.com"} "${story.title}"`)}`;
const sourceActionForStory = (story:Story) => story.articleUrl ? "Read original" : "Find source article";
...
export default function Home(){
  ...
  return <main className="app-shell">
    ...
    {selectedStory&&<div className="story-modal-backdrop" role="presentation" onMouseDown={()=>setSelectedStory(null)}><article className="story-modal" role="dialog" aria-modal="true" aria-labelledby="story-modal-title" onMouseDown={(event)=>event.stopPropagation()}><button className="modal-close" onClick={()=>setSelectedStory(null)} aria-label="Close briefing">×</button><p>{selectedStory.category} · {selectedStory.region}</p><h2 id="story-modal-title">{selectedStory.title}</h2><div className="modal-meta"><span className={`status-dot ${selectedStory.level}`}/>{selectedStory.source} · {selectedStory.time} ago · {selectedStory.read} read</div><p className="modal-summary">{selectedStory.summary}</p><div className="modal-tags">{selectedStory.tags.map((tag)=><button key={tag} onClick={()=>{setQuery(tag);setAppliedQuery(tag);setSelectedStory(null);setShowAll(true);requestAnimationFrame(()=>navigateTo("Briefings"));}}>#{tag}</button>)}</div><div className="modal-actions"><a className="primary-action" href={sourceUrlForStory(selectedStory)} target="_blank" rel="noreferrer">{sourceActionForStory(selectedStory)} ↗</a><button onClick={()=>toggleSaved(selectedStory.id)}>{savedIds.includes(selectedStory.id)?"◆ Remove saved":"◇ Save"}</button><button onClick={()=>setSelectedStory(null)}>Close</button></div><small className="demo-note">This is a demonstration briefing. Because its headline is sample data, “Find source article” runs an exact-title search limited to the named publisher; add an articleUrl to a story to make the action link directly to its verified original.</small></article></div>}
  </main>;
}
